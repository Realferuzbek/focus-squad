...
function EmptyRow(): Block {
  return { start: "10:00", end: "12:00", label: "" };
}
...
export default function AdminSettings() {
  ...
  const [rows, setRows] = useState<Block[]>([
    EmptyRow(),
    { start: "19:00", end: "21:00", label: "Evening" },
  ]);
  ...
  async function load() {
    const s = await fetch("/api/admin/schedule", { cache: "no-store" }).then(
      (r) => r.json(),
    );
    if (s.template?.blocks) setRows(s.template.blocks);

    const u = await fetch("/api/admin/users", { cache: "no-store" }).then((r) =>
      r.json(),
    );
    setUsers(u.users ?? []);
    setAllow(u.allowlist ?? []);
  }
  ...
  async function saveOverride(del = false) {
    ...
    const r = await csrfFetch("/api/admin/schedule/override", {
      ...
      body: JSON.stringify(
        del
          ? { date: overrideDate, delete: true }
          : { date: overrideDate, blocks: overrideRows },
      ),
    });
    setLoading(false);
    if (!r.ok) alert("Failed");
    else alert(del ? "Override removed" : "Override saved");
  }
    ...
    <div className="space-y-8">
      ...
      <section className="bg-card rounded-2xl p-4 border border-neutral-800">
        ...
        <div className="flex items-center gap-3 mb-3 text-sm">
          ...
          <input
            type="date"
            className="bg-[#0f0f13] border border-neutral-800 rounded-xl p-2 focus-ring"
            value={activeFrom}
          ...
          {rows.map((b, i) => (
            <div key={i} className="flex items-center gap-2 text-sm">
              <input
                ...
                className="bg-[#0f0f13] border border-neutral-800 rounded-xl p-2 focus-ring w-36"
              />
              <input
                value={b.start}
                onChange={(e) => setRow(i, { start: e.target.value })}
                className="bg-[#0f0f13] border border-neutral-800 rounded-xl p-2 focus-ring w-24"
                placeholder="HH:MM"
              />
              <span>–</span>
              <input
      ...
      <section className="bg-card rounded-2xl p-4 border border-neutral-800">
        ...
        <div className="flex items-center gap-3 mb-3 text-sm">
          ...
          <button
            className="btn-secondary focus-ring text-xs"
            onClick={() => setOverrideRows((r) => [...r, EmptyRow()])}
          >
            + Add row
          </button>
        </div>
        <div className="space-y-2">
          {overrideRows.map((b, i) => (
            <div key={i} className="flex items-center gap-2 text-sm">
              <input
                value={b.label ?? ""}
                onChange={(e) => setORow(i, { label: e.target.value })}
                placeholder="Label"
                className="bg-[#0f0f13] border border-neutral-800 rounded-xl p-2 focus-ring w-36"
              />
              <input
                value={b.start}
                onChange={(e) => setORow(i, { start: e.target.value })}
                className="bg-[#0f0f13] border border-neutral-800 rounded-xl p-2 focus-ring w-24"
                placeholder="HH:MM"
              />
              <span>–</span>
              <input
                value={b.end}
                onChange={(e) => setORow(i, { end: e.target.value })}
                className="bg-[#0f0f13] border border-neutral-800 rounded-xl p-2 focus-ring w-24"
                placeholder="HH:MM"
              />
              <button
                className="btn-secondary focus-ring text-xs"
                onClick={() =>
                  setOverrideRows((r) => r.filter((_, x) => x !== i))
                }
              >
                Remove
              </button>
      ...
      <section className="bg-card rounded-2xl p-4 border border-neutral-800">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <button
            className="btn-secondary focus-ring text-xs"
            onClick={() => action("allowlist:add", email)}
          >
            Add to admin allow-list
          </button>
          <button
            className="btn-secondary focus-ring text-xs"
            onClick={() => action("allowlist:remove", email)}
          >
            Remove from allow-list
          </button>
          <button
            className="btn-secondary focus-ring text-xs"
            onClick={() => action("admin:promote", email)}
          >
            Promote now
          </button>
          <button
            className="btn-secondary focus-ring text-xs"
            onClick={() => action("admin:demote", email)}
          >
            Demote
          </button>
        </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {users.map((u: any) => (
                <li
                  key={u.email}
                  className="border border-neutral-800 rounded-xl p-2 flex items-center justify-between"
                >
                  <span>
                    {u.display_name ?? u.email}{" "}
                    <span className="text-subtle">({u.email})</span>
                  </span>
                  <span className="flex items-center gap-2">
                    ...
                    <button
                      className="btn-secondary focus-ring text-xs"
                      onClick={() =>
                        action("user:block", u.email, !u.is_blocked)
                      }
                    >
            ...
            <ul className="space-y-2 text-sm">
              {allow.map((a: any) => (
                ...
                >
                  ...
                  <button
                    className="btn-secondary focus-ring text-xs"
                    onClick={() => action("allowlist:remove", a.email)}
                  >
                    Remove
                  </button>
                </li>
