...
assert(/secure:\s*SESSION_COOKIE_SECURE/.test(content), 'secure flag must use SESSION_COOKIE_SECURE');

// Check sid generation & rotation hints
assert(/sidIssuedAt/.test(content), 'sidIssuedAt tracking must be present');
assert(/mintSessionState/.test(content), 'mintSessionState helper should be used');
