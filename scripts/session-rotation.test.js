...
(function testResolveSessionRollingIntervalDefaults() {
  ...
  assert.strictEqual(explicitMs, 30 * MINUTES_TO_MS, "explicit minutes should be converted to ms");
})();
...
(function testNeedsRollingRotation() {
  ...
  assert.strictEqual(needsRollingRotation(null, now, interval), true, "missing issuedAt should force rotation");
  assert.strictEqual(needsRollingRotation(now - interval - 1000, now, interval), true, "expired session should rotate");
  assert.strictEqual(needsRollingRotation(now - interval + 1000, now, interval), false, "fresh session should not rotate");
})();
