  ...
  try {
    ...
    const scopeEntries = await Promise.all(
      REQUIRED_SCOPES.map(async (scope) => {
        ...
        const withinWindow = Number.isFinite(latestTimestamp) && latestTimestamp >= windowStart;
    ...
    const scopes = Object.fromEntries(scopeEntries) as Record<(typeof REQUIRED_SCOPES)[number], ScopeSummary>;

    let latestPostedAt: string | null = null;
    let ok = true;

    Object.values(scopes).forEach((summary) => {
      if (summary.postedAt && (!latestPostedAt || summary.postedAt > latestPostedAt)) {
    ...
    if (!ok) {
      console.error('leaderboard health: stale or missing snapshots detected', scopes);
    } else {
