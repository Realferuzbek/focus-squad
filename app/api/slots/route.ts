...
export async function GET(req: NextRequest) {
  ...
  const date =
    ...
    DateTime.now().setZone(TASHKENT).toISODate()!;
  ...
  const noteMap = new Map<number, string>(
    (notes ?? []).map((n: any) => [n.slot_index as number, n.note as string]),
  );
...
export async function POST(req: NextRequest) {
  ...
  const hasStarted =
    DateTime.now().setZone(TASHKENT) >= slotStart(date, blocks[index].start);
