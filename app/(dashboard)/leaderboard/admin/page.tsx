...
async function loadSummaries(): Promise<ScopeSummary[]> {
  ...
  const summaries = await Promise.all(
    REQUIRED_SCOPES.map(async (scope) => {
      ...
      if (error) {
        console.error("leaderboard admin: failed to load summary", {
          scope,
          error,
        });
        throw error;
...
async function simulateIngestAction() {
  ...
  if (!process.env.LEADERBOARD_INGEST_SECRET) {
    console.warn(
      "leaderboard admin: simulation failed because secret is missing",
    );
...
export default async function LeaderboardAdminPage({
  ...
  const message =
    typeof searchParams?.simulated === "string"
      ? "Simulation payload ingested successfully."
      : null;
  const error =
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-6 p-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Review the latest stored leaderboard snapshots and trigger a local
            simulation.
          </p>
        </div>
        {allowSimulation ? (
      ...
      {message ? (
        <p className="rounded border border-green-600 bg-green-50 p-3 text-sm text-green-700">
          {message}
        </p>
      ) : null}
      {error ? (
