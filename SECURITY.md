...
- We add a double-submit CSRF cookie (non-HttpOnly) to support SPA fetch flows; the server verifies `X-CSRF-Token` header matches the CSRF cookie for state-changing requests.
...
Testing tips:
- Run `node ./scripts/session-cookie.test.js` and `node ./scripts/csrf.test.js` locally to validate cookie attributes and CSRF token helpers.
