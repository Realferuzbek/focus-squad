...
function resolveSessionRollingInterval(envMinutesValue) {
  if (!envMinutesValue) return DEFAULT_SESSION_ROLLING_INTERVAL_MINUTES * MINUTES_TO_MS;
...
function needsRollingRotation(lastIssuedAt, now, intervalMs) {
  ...
  if (typeof lastIssuedAt !== "number" || !Number.isFinite(lastIssuedAt) || lastIssuedAt <= 0) {
    return true;
  }
