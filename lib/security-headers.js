...
const RAW_IFRAME_ANCESTORS =
  process.env.SECURITY_IFRAME_ANCESTORS || "";
const EXTRA_IFRAME_ANCESTORS = RAW_IFRAME_ANCESTORS.split(",")
  .map((value) => value.trim())
  .filter((value) => value.length > 0);

const VALID_CORP_VALUES = new Set([
  "same-origin",
  "same-site",
  "cross-origin",
]);
...
const CSP_REPORT_ONLY = serializeDirectives(BASE_CSP_DIRECTIVES);
...
function buildContentSecurityPolicy(context = {}) {
  const directives = BASE_CSP_DIRECTIVES.map((directive) => {
    if (!directive.value) return { ...directive };
    return { name: directive.name, value: [...directive.value] };
  });
