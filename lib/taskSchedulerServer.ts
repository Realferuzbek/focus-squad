...
export function serializeTask(row: TaskItemRow): StudentTask {
  return {
    ...
    subject: (row.subject as StudentTask["subject"]) ?? null,
    resourceUrl: (row.resource_url as StudentTask["resourceUrl"]) ?? null,
    ...
    estimatedMinutes:
      (row.estimated_minutes as StudentTask["estimatedMinutes"]) ?? null,
    repeatRule: "none",
...
export function serializeHabit(row: TaskHabitRow): StudentHabit {
  return {
    ...
    target: (row.target as StudentHabit["target"]) ?? null,
    notes: row.notes ?? null,
    resourceUrl: (row.resource_url as StudentHabit["resourceUrl"]) ?? null,
